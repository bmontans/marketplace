...
async function newUser(req, res, next) {
  ...
  try {
    ...
    const { name, address, email, birthdate, password } = req.body;

    const [
      existingUsername,
    ] = await connection.query("SELECT pk_id FROM user WHERE name=?", [name]);
    if (existingUsername.length) {
      throw generateError("The name already exists on the DB", 409);
    }
    const [
      existingEmail,
    ...
    await connection.query(
      `INSERT INTO user ( name, address, email, birthdate, password, creation_date, modification_date, role, active, registrationcode)
      VALUES (?,?,?,?,?,NOW(),NOW(),"normal",false,?) `,
      [name, address, email, birthdate, dbPassword, registrationCode]
    );
